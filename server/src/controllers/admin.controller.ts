...
export class AdminController extends BaseController<any> {
  ...
  public createBusiness = async (req: Request, res: Response) => {
    try {
      ...
      if (existingBusiness) {
        return this.sendError(res, 'Business with this email already exists', 400);
      }

      const business = await Business.create(businessData);
      
      logger.info(`Admin created new business: ${business.name} (${business.email})`);
  ...
  public deleteBusiness = async (req: Request, res: Response) => {
    try {
      ...
      await ApiKey.updateMany(
        { business: business._id },
        { status: 'revoked', revokedAt: new Date(), revokedReason: 'Business deleted' }
      );
  ...
  public revokeApiKey = async (req: Request, res: Response) => {
    try {
      ...
      logger.info(`Admin revoked API key: ${apiKey.name} for business ${req.params.businessId}`);
  ...
  public getPlatformStatistics = async (_req: Request, res: Response) => {
    try {
      ...
      const [newBusinessesThisMonth, newBusinessesLastMonth] = await Promise.all([
        Business.countDocuments({
          createdAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }
        }),
        Business.countDocuments({
  ...
  public suspendBusiness = async (req: Request, res: Response) => {
    try {
      ...
      logger.info(`Admin suspended business: ${business.name} - Reason: ${req.body.reason || 'No reason provided'}`);
